...
mix.sass(`resources/assets/core/plugins/plugins.scss`, `public/${folderAssets}/plugins/global/plugins.bundle.css`).then(() => {
    ...
    .scripts(require('./resources/assets/core/plugins/plugins.js'), `public/${folderAssets}/plugins/global/plugins.bundle.js`);

// Build extended plugin styles
mix.sass(`resources/assets/${demo}/sass/plugins.scss`, `public/${folderAssets}/plugins/global/plugins-custom.bundle.css`);

// Build Metronic css/js
mix.sass(`resources/assets/${demo}/sass/style.scss`, `public/${folderAssets}/css/style.bundle.css`, {sassOptions: {includePaths: ['node_modules']}})
    // .options({processCssUrls: false})
    .scripts(require(`./resources/assets/${demo}/js/scripts.js`), `public/${folderAssets}/js/scripts.bundle.js`);


// Build custom 3rd party plugins
(glob.sync(`resources/assets/core/plugins/custom/**/*.js`) || []).forEach(file => {
    mix.js(file, `public/${folderAssets}/${file.replace(`resources/assets/core/`, '').replace('.js', '.bundle.js')}`);
});
(glob.sync(`resources/assets/core/plugins/custom/**/*.scss`) || []).forEach(file => {
    mix.sass(file, `public/${folderAssets}/${file.replace(`resources/assets/core/`, '').replace('.scss', '.bundle.css')}`);
});

// Build Metronic css pages (single page use)
(glob.sync(`resources/assets/${demo}/sass/pages/**/!(_)*.scss`) || []).forEach(file => {
    file = file.replace(/[\\\/]+/g, '/');
    mix.sass(file, file.replace(`resources/assets/${demo}/sass`, `public/${folderAssets}/css`).replace(/\.scss$/, '.css'));
});

// var extendedFiles = [];
// Extend custom js files for laravel
// (glob.sync('resources/assets/extended/js/**/*.js') || []).forEach(file => {
//     var output = `public/${folderAssets}/${file.replace('resources/assets/extended/', '')}`;
...
//     var output = `public/${folderAssets}/${file.replace('resources/assets/core/', '')}`;
//     if (extendedFiles.indexOf(output) === -1) {
//         mix.js(file, output);
//     }
// });
// (glob.sync(`resources/assets/${demo}/js/custom/**/*.js`) || []).forEach(file => {
//     var output = `public/${folderAssets}/${file.replace(`resources/assets/${demo}/`, '')}`;
//     if (extendedFiles.indexOf(output) === -1) {
//         mix.js(file, output);
//     }
// });
...
// Metronic media
mix.copyDirectory('resources/assets/core/media', `public/${folderAssets}/media`);
mix.copyDirectory(`resources/assets/${demo}/media`, `public/${folderAssets}/media`);
...
let plugins = [
    new ReplaceInFileWebpackPlugin([
        {
            // rewrite font paths
            dir: path.resolve(`public/${folderAssets}/plugins/global`),
            test: /\.css$/,
            rules: [
                {
                    // fontawesome
...
(glob.sync(`resources/assets/core/plugins/**/*.+(woff|woff2|eot|ttf|svg)`) || []).forEach(file => {
    ...
    mix.copy(file, `public/${folderAssets}/plugins/global/fonts/${folder}/${path.basename(file)}`);
});
...
(glob.sync('node_modules/jstree/dist/themes/default/*.+(png|gif)') || []).forEach(file => {
    mix.copy(file, `public/${folderAssets}/plugins/custom/jstree/${path.basename(file)}`);
});
...
(glob.sync(`resources/assets/core/plugins/custom/**/*.js.json`) || []).forEach(file => {
    ...
    mix.scripts(filePaths, `public/${folderAssets}/plugins/custom/${fileName}/${fileName}.bundle.js`);
});

mix.scripts((glob.sync(`resources/assets/core/js/widgets/**/*.js`) || []), `public/${folderAssets}/js/widgets.bundle.js`);
